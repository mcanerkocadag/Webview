...
class ShadowLayout : FrameLayout {
    ...
    private var shadowSize: Float = 0.toFloat()
    ...
    private fun initAttributes(attrs: AttributeSet?) {
        val attr = context.obtainStyledAttributes(attrs, R.styleable.ShadowLayout, 0, 0) ?: return

        try {
            cornerRadius = attr.getDimension(
                R.styleable.ShadowLayout_slCornerRadius,
                resources.getDimension(R.dimen.defaultMenuDropShadowCornerRadius)
            )
            shadowSize = attr.getDimension(
                R.styleable.ShadowLayout_slShadowSize,
    ...
    private fun createShadowBitmap(
        ...
        shadowSize: Float, dx: Float, dy: Float, shadowColor: Int, fillColor: Int
    ): Bitmap {
